...
my $plugin = MT::Plugin::YearEntry->new({
    ...
    registry => {
      tags => {
        ...
        block => {
          #'EntriedYears' => \&hdler_EntriedYears,
...
sub _getEntryYear {
  ...
  if( $offset_year < 1900 ) {
    ...
    $year = $current_authored_on - abs($offset_year);
  }
  else {
    $year = $offset_year;
  }
...
#sub hdler_EntriedYears {
#  my ( $ctx, $args, $cond ) = @_;
#  my $tag = 'MTEntriedYears';
#
#
#  my $blog_id = $ctx->stash("blog_id");
#  my $term = { blog_id => $blog_id };
#  my $arg = {
#    'sort' => 'authored_on',
#    direction => 'ascend',
#    limit => 1,
...
#  };
#
#  $iter = MT::Entry->load_iter($term, $arg) or return '';
#  my $last_year;
#  while( my $entry = $iter->() ){
#    $last_year = $entry->authored_on;
#  }
#
#  my $out;
#  my $i = $first_year;
#  while( $i <= $last_year ){
#    local $ctx->{__stash}{EntriedYear} = $i;
#
#    my $tokens = $ctx->stash('tokens');
#    my $builder = $ctx->stash('builder');
#    $out .= $builder->build($ctx, $tokens) or return $ctx->error($builder->errstr);
